...
async function main() {
  try {
    const maxPagesEnv = Number(process.env.BOE_MAX_ITEMS || process.env.BOE_MAX_PAGES || "50");
    const maxPages = Number.isFinite(maxPagesEnv) && maxPagesEnv > 0 ? Math.min(Math.max(Math.floor(maxPagesEnv), 50), 100) : 50;
